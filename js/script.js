...
function dinheiroParaFloat(sal) {
  return parseFloat(sal.replace(".", "").replace("R$ ", "").replace(",", "."));
}
...
function calculaDescontoInss(valor, aliquota) {
  return (valor / 100) * aliquota;
}

function calculaDescontoIrpf(valor, aliquota, deduzir) {
  return (valor / 100) * aliquota - deduzir;
}
...
function calculaSalario(sal) {
  ...
  if (valor <= inss.faixa1.max) {
    imposto.descontoInss = calculaDescontoInss(valor, inss.faixa1.aliquota);
    inss.faixa1.ativo = true;
  } else if (valor >= inss.faixa2.min && valor <= inss.faixa2.max) {
    imposto.descontoInss = calculaDescontoInss(valor, inss.faixa2.aliquota);
    inss.faixa2.ativo = true;
  } else if (valor >= inss.faixa3.min) {
    imposto.descontoInss = calculaDescontoInss(valor, inss.faixa3.aliquota);
  ...
  if (valor <= irpf.faixa1.max) {
    ...
    imposto.descontoIrpf = calculaDescontoIrpf(
      valor,
      irpf.faixa2.aliquota,
      irpf.faixa2.deduzir
    );
